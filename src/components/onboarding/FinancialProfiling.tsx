...
export const FinancialProfiling = (
  { onComplete, skipLanguageStep = false }: FinancialProfilingProps
) => {
  const { updateFinancialProfile, financialProfile, setStep } = useOnboarding();
  const { language, setLanguage } = useLanguage();
  const totalSubSteps = skipLanguageStep ? 6 : 7;
  const [subStep, setSubStep] = useState(skipLanguageStep ? 1 : 2);
      ...
      >
        ...
        <motion.div variants={itemVariants} className="pt-4 flex space-x-4">
          <Button onClick={handleNext} className="flex-1 bg-gradient-primary">
            {language === "fr" ? "Suivant" : language === "ar" ? "التالي" : "Next"} <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </motion.div>
      </motion.div>
    );
  ...
  if (subStep === (skipLanguageStep ? 1 : 2)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
  ...
  if (subStep === (skipLanguageStep ? 2 : 3)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
  ...
  if (subStep === (skipLanguageStep ? 3 : 4)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
  ...
  if (subStep === (skipLanguageStep ? 4 : 5)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
  ...
  if (subStep === (skipLanguageStep ? 5 : 6)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
  ...
  if (subStep === (skipLanguageStep ? 6 : 7)) {
    return (
      <motion.div
        variants={containerVariants}
        initial="hidden"
